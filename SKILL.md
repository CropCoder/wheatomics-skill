...
### Genes
- `GET /api/genes/detail/{gene_id}` — GeneHub: gene detail with description, chromosome, protein length, MW, pI, annotations. 支持 `TraesCS` 格式（01G/02G/03G，如 `TraesCS5A02G391700`、`TraesCS5A02G391700.1`），不支持 `Traes_5AL_*` 旧格式。不支持格式的 ID 先用 `id-conversion` 转换。
...
- `GET /api/genes/functions/registry` — List all registered genome/function tables with metadata (table_name, NameGroup, Polyploidy, chromosome_level, DOI, title, Abstract, example IDs).
...
- `GET /api/ppi/query?gene_ids=&table=&min_score=` — 查询蛋白互作（CF-MS 评分）。数据表: `PPI_result`。仅支持中国春 02G 转录本 ID（需加 `.1` 后缀），不支持 eggNOG ID。评分阈值: 0.5（高置信）/ 0.2（中等）/ 0（全部）。
